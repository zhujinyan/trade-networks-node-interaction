...
def get_order(hs, time, target, indicator):
    ...
    region_order = [row[0] for row in rows]

    return region_order


def read_csv(path, hs, time, target):
    df = pd.read_csv(os.path.join(path, f'{hs}_2021_results.csv'))

    data = df[(df['time'] == time) & (df['target'] == target)]
    y = list(data.sort_values('influence', ascending=False)['attacker'])
...
def main():
    ...
    for i in range(1, 2):
        ...
        y = read_csv(folder_path, hs, time, target)
    ...
    df.to_excel('87_2021.xlsx', index=False)
