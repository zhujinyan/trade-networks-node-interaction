...
if __name__ == '__main__':
    ...
    for i in range(1, 4):
        ...
        for hs in range(87, 88):
            ...
            with open(f'{hs}_sub{i}_y_corrcoef.csv', 'w', encoding='utf-8', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['time', 'target', 'indicator', 'corrcoef'])

            with open(f'{hs}_sub{i}_auc_corrcoef.csv', 'w', encoding='utf-8', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['time', 'target', 'indicator', 'corrcoef'])
            ...
            for indicator in indicators:
                for target in ['china']:
                    print(f'{hs} {time} {target} {indicator}')
                    order = get_order(hs, time, target, indicator)

                    x = [row[1] for row in order]
                    y, auc = read_csv(folder_path, hs, time, target, order)

                    #graph(x, y, hs, time, target, indicator)
                    cc_y = corrcoe(x, y)
                    cc_auc = corrcoe(x, auc)
                    with open(f'{hs}_sub{i}_y_corrcoef.csv', 'a', encoding='utf-8', newline='') as f:
                        writer = csv.writer(f)
                        writer.writerow([time, target, indicator, cc_y[0][1]])

                    with open(f'{hs}_sub{i}_auc_corrcoef.csv', 'a', encoding='utf-8', newline='') as f:
                        writer = csv.writer(f)
                        writer.writerow([time, target, indicator, cc_auc[0][1]])
